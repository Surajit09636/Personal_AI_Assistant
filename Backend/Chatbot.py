...
def RealTimeInformation():
    ...
    month = current_date_time.strftime("%B")  # Full name of the month
...
def ChatBot(Query):
    ...
    try:
        ...
        with open(r"Data\ChatLog.json", "r") as f:
            messages = load(f)  # Use messages instead of message to maintain consistency

        # Append the user's query to the messages list
        messages.append({"role": "user", "content": f"{Query}"})

        completion = client.chat.completions.create(
            model="llama3-70b-8192",  # Corrected spelling from "moel" to "model"
            messages=SystemChatBot + [{"role": "system", "content": RealTimeInformation()}] + messages,  # Fixed incorrect "rple" key
