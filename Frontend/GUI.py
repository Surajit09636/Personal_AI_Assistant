...
def QueryModifier(Query):
    ...
    questions_words = ["how", "what", "who", "where", "when", "why", "which", "whose", "whom", "can you", "what's", "where's", "how's"]
...
class ChatSection(QWidget):
    def __init__(self):
        ...
        movie = QMovie(GraphicsDirectoryPath("face.gif"))
        max_gif_size_W = 480
        max_gif_size_H = 270
...
class CustomTopBar(QWidget):
    ...
    def showInitializeScreen(self):
        if self.current_screen is not None:
            self.current_screen.hide()
            
        initialize_screen = InitialScreen(self)
        layout = self.parent().layout()
        if layout is not None:
            layout.addWidget(initialize_screen)
        self.current_screen = initialize_screen
